...
<%--__boardTypeList는 인터셉터로 어플리케이션에 올려놨기 때문에 바로 사용 가능함
    하지만 boardCode는 아님. -> pathvariable 이용해서 request scope에 올려주기--%>
...
<body>
    <main>
        ...
        <section class="board-detail">  
            ...
            <c:if test="${not empty board.imageList}">
                ...
                <%-- 썸네일이 있을 경우 변수 생성 _첫번째 이미지는 무조건 썸네일(0번)--%>
                ...
                <c:if test="${not empty thumbnail}"> <%-- 썸네일O --%>
                    <c:set var="start" value="1"/>
                </c:if>

                            <%-- __썸네일 하나만 imageList에 있을 경우 start 실행 안함(length==1) 1보다 크지않음--%>
    ...
    <%-- __단점: 값을 변형시킬 수 있고, 값을 얻어가는 코드를 또 작성해야 함. --%>
    <%-- <input type="hidden" name="memberNo" value="${loginMember.memberNo}">       --%>
    
    <%-- JSP 내장 객체에 세팅 값을 JS에서 얻어가는 방법 2 --%>
    <%-- : sciprt 태그를 이용해서 전역변수로 선언하는 방법 --%>
    <script>
        // 로그인한 회원 번호 얻어오기
        // (참고) JSP 해석 순서 : EL/JSTL > HTML > JS
        // *** JS에 EL/JSTL 사용 시 양쪽에 "" 또는 ''를 붙이는 것을 권장 ***
        // -> 왜? 값이 없어서 공백이 되더라도 ""(빈문자열)로 인식하여 에러 발생을 막음.
