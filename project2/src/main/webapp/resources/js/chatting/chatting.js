...
if(loginMemberNo != ""){
	...
	//_객체가 만들어지자마자 연결되는 주소 - 스프링 servlet-context에 웹소켓 매핑 주소가 있음.
	// -> 매피중소는 websocketHandler로 연결해줌 -> 우리가 만든 클래스
}
...
chattingSock.onmessage = function(e) {
	...
	const msg = JSON.parse(e.data);   //_JSON 객체를 js객체로
	console.log(msg);
	...
	if(selectChattingNo == msg.chattingNo){
		...
		//_보낸사람, 받는사람이 동시에 메세지를 받으므로 나눠서 구분
		// 내가 작성한 메세지인 경우
		if(loginMemberNo == msg.senderNo){ 
			li.classList.add("my-chat");
