...
//	@RequestMapping(value="/member/login", method=RequestMethod.POST)
	public String login(@RequestParam("inputEmail") String email, //_메서드명이 같지만 매개변수가 달라 에러가 x (오버로딩)
						@RequestParam(value="inputPw2", required=false, defaultValue="1234") String pw,
						String inputPw) {  
														//_required=true가 기본값
		System.out.println(email);
		System.out.println(pw);
		System.out.println(inputPw);  //_@RequestParam 생략한 경우, 매개변수랑 name 속성 값 이름이 같아서 생략 가능.
...
//	@RequestMapping(value="/member/login", method=RequestMethod.POST)
	...
	*/
	...
	//_똑같은 주소에, 똑같은 요청방식 중복xxxx																										
	@PostMapping("/member/login")   							//_HttpSession을 이용해 세션 객체 얻어와지지만 스프링에서는 다른 걸(Model) 사용.
	public String login(/*@ModelAttribute*/ Member inputMember, Model model, RedirectAttributes ra, 
						@RequestParam(value="saveId", required=false) String saveId,  // 체크박스 값 얻어오기   _name이 saveId인 값을 얻어와서 체크박스에. required= 기본값 true(체크안하면 null이 들어와 에러) -> required 설정해야함.	
		...
		if(loginMember != null) {
			...
			if(saveId != null) {  // 체크되었을 때
				...
				// 1년동안 쿠키 유지
				cookie.setMaxAge(60 * 60 * 24 * 365);  //( s *  m * h * d)
	...
	public String loginPage() {
		return "member/login";  //_servlet-context -> view resolver 적용 -> 앞뒤주소 생략하고 작성. forward.
	}
	...
	public String logout(SessionStatus status) {
		...
		status.setComplete();  // 세션 무효화 cleanup
	...
	@PostMapping("/member/signUp")   /* 매개변수에 @ModelAttribute 생략*/ 
