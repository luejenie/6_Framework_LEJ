...
//			return result;
	...
	public int updateProfile(String webPath, String filePath, MultipartFile profileImage, Member loginMember) 
		...
		if(profileImage.getSize() == 0) { // 업로드된 파일이 없는 경우
			...
			rename = Util.fileRename( profileImage.getOriginalFilename() );   //_Util은 common 폴더에 만듦
			loginMember.setProfileImage(webPath + rename);
			// '/resources/images/memberProfile/변경된파일명'  __-> 회원의 프로필이미지 경로
		}
		...
		// DB 성공 -> 파일 업로드 실패
		// DB 실패 -> 파일 업로드 성공
		//_이런 경우도 생각해야함.  -> 위 Transactional(rollbackFor~~)로 수행
		...
		if( result > 0) { // DB 수정 성공 시 -> 실제로 서버에 파일 저장
			...
			if(rename != null) {
				...
				// 메모리에 임시 저장된 파일을 지정된 경로에 파일 형태로 변환  //_여기서 예외발생할 확률 높음
				// == 서버 파일 업로드
			}
		} else {
			// 실패 시 다시 이전 이미지를 세팅
