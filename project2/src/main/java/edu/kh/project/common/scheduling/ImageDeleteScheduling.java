...
//__ @Component 어떤 목적인지는 모르겠으나 bean등록을 하겠다.
...
//	@Scheduled(fixedRate = 10000)  // 이전 작업이 시작 후 10초 뒤에 수행
	...
	// crom="초 분 시 일 월 요일 [년도]"
//	@Scheduled(cron = "0,15,30,45 * * * * *")  // 0초부터 시작하여 15초마다 // 0,30 띄어쓰기 없이 작성, 매분 0초 30초마다
//	@Scheduled(cron = "0/15 * * * * *")  // 0초부터 시작하여 15초마다
...
//		System.out.println("0초마다 출력");
		...
		// 2. Server에 저장된 모든 이미지 파일 조회 (images>board)
		String folderPath = application.getRealPath("/resources/images/board");  //_실제 컴퓨터상의 주소로 접근
		...
		if(!fileList.isEmpty()) { // 서버에 파일이 있다면
			...
			for(File file : fileList) {
												//_제일 마지막 숫자만 가져오도록!
				// file.toString();  // 파일이 저장된 경로 반환 ex) C:\workspace\6_Framwork\project2 .....\20221129100447_79329.jpg
				// 문자열.substring(인덱스): 지정된 인덱스부터 끝까지 잘라서 반환
...
//					System.out.println(fileName + "삭제");
					...
					// 로그에는 trace, debug, info, warn, error, fatal
