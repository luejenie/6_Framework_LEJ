...
public class ChattingWebsocketHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// WebSocketSession session : 클라이언트와 서버 간의 전이중통신을 담당하는 객체
		// + 웹소켓에 접속한 회원의 HttpSession을 훔쳐서 가지고 있음
		
		sessions.add(session);
		// 현재 채팅방에 접속한 회원의 세션을 모아둠.  //_-> 특정한 사람을 찾는게 가능해짐	
	}

	// handleTextMessage : 클라이언트로부터 텍스트 메세지를 받으면 수행되는 메서드
	// --> JS의 chattingSock.send(JSON데이터);
	// --> handleTextMessage가 수행
	//     --> message.getPayLoad() 수행하면 값 전달 받음== JSON데이터 
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		if(result > 0) { // 삽입 성공 시
			...
			// 보낸 시간이 DB에 있고 msg 객체에는 없는 상태
			// -> 보낸 시간 생성
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd hh:mm");
			msg.setSendTime(sdf.format(new Date()));  //_보낸 시간이 msg에 세팅
            ...
            for(WebSocketSession s : sessions) {  //_채팅방 sessions에서 한명을 꺼내서 s라고 하자.
                ...
                if(loginMemberNo == msg.getTargetNo() || loginMemberNo == msg.getSenderNo()) {
                    ...
                    //_ msg에 담긴 내용(채팅방번호, 대상번호, 보낸사람번호, 내용, 보낸시간)등을 Gson을 이용해서
                    // Json으로 바꿔서 보냄.  -> chatting.js의 293번줄로 감.
                }
            }
...
- 브라우저와 웹서버간의 전이중통신을 지원하는 프로토콜이다.  
    / ==양방향통신 / 웹서버 = 기지국
	/ 서버에서 접속한 모든 사람의 session 객체를 관리함 
	/ -> 메세지 보내는 특정 객체 관리가 가능
- HTML5버전부터 지원하는 기능이다.
...
TextWebSocketHandler :  WebSocketHandler 인터페이스를 상속받아 구현한 텍스트 메세지 전용 웹소켓 핸들러 클래스 (子)
